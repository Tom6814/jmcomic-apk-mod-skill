...
description: "JMComic3 APK 逆向修改：去除广告、移除游戏/电影板块。支持仅去广告、去广告+去板块，以及灵活的自定义组合。基于 v2.0.29 实战经验。触发词：JMComic去广告、APK修改、去除游戏电影、jmcomic mod"
---
...
两个预设分支覆盖了最常见的场景。如果你有特殊需求，可以直接从以下映射表中选择需要的步骤进行组合。

### adKey → 步骤映射

每个广告位对应一个修改步骤，你可以按需选取：
...
| 目标 | 步骤 | 影响 |
|------|------|------|
| 移除游戏板块 | B1 + B2 + B3 + B4 + B5 | 仅删游戏路由/chunk |
| 移除电影板块 | B1 + B2 + B3 + B4 + B5 | 仅删电影路由/chunk |
| 全部移除 | B1-B5 | 游戏+电影 |

### 常见自定义组合

**场景 1：「我只想去掉 banner 和文字链接，闪屏留着」**
→ 执行 A1 + A2 + A7

**场景 2：「去掉所有广告，但保留游戏板块，删除电影板块」**
→ 执行全部 A 步骤 + B1(仅电影路由) + B2(仅电影 tab) + 孤儿清理(仅电影 chunk)

**场景 3：「先只研究广告结构，不做任何修改」**
→ 只执行「适配新版本的标准侦查流程」

**场景 4：「去广告 + 去电影，但保留游戏」**
→ 执行全部 A 步骤 + B1(删 `/movies` 和 `/movies/:id`) + B2(仅电影 tab) + 孤儿清理(仅 7521、6120)

**场景 5：「去广告，不改 chunk 结构，只屏蔽广告渲染」**
→ 执行 A1-A6 + A7 + A9（不做任何 chunk 删除）
